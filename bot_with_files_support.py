...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if update.message.photo:
            ...
            file_data = file_buffer.getvalue()  # Данные файла для отладки
            logger.info(f"Размер файла: {len(file_data)} байт")

            # Кастомная загрузка файла через HTTP
            async with httpx.AsyncClient(verify=False) as client:
                response = await client.post(
                    ...
                    files={"file": ("photo.jpg", file_data, mime_type)},
                    params={"purpose": "general"}
                )
                logger.info(f"Заголовки запроса: {dict(response.request.headers)}")
        ...
        # Проверка на документ
            ...
            async with httpx.AsyncClient(verify=False) as client:
                response = await client.post(
                    ...
                    files={"file": (document.file_name, file_data, mime_type)},
                    params={"purpose": "general"}
                )
                logger.info(f"Заголовки запроса: {dict(response.request.headers)}")
                response.raise_for_status()
        ...
        logger.error(f"Ошибка при загрузке файла: {e.response.text}")
        await update.message.reply_text("Извини, произошла ошибка при загрузке файла в GigaChat.")
    except Exception as e:
